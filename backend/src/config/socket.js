...
const initializeSocket = (server) => {
  ...
  io.use((socket, next) => {
    ...
    // If admin, join admin room for live tracking and alerts
    if (socket.userRole === 'admin') {
      socket.join('admin');
      console.log(`🛡️ Admin ${socket.userId} joined admin room`);
    }
    ...
    socket.on('location:update', (data) => {
      ...
      // Broadcast to admin dashboard for live tracking
      io.to('admin').emit('admin:location_update', {
        driverId,
        location,
        timestamp: Date.now()
      });
    ...
    socket.on('sos:trigger', (data) => {
      const { bookingId, location, details } = data;
      console.log(`🚨 SOS Triggered by ${socket.userId} (${socket.userRole}) for ride ${bookingId}`);
      
      // Broadcast high-priority alert to all admins
      io.to('admin').emit('admin:sos_alert', {
      ...
      // Optionally notify the other party in the ride
      if (bookingId) {
        io.to(`ride:${bookingId}`).emit('ride:sos_alert', {
          initiatedBy: socket.userRole,
          timestamp: Date.now()
        });
      }
    });
