...
export default function LiveMap() {
  ...
  useEffect(() => {
    fetchMapData();
    const interval = setInterval(fetchMapData, 60000); // reduced frequency since we have sockets

    const token = localStorage.getItem('adminToken');
    // Determine socket URL based on current origin for correct behavior in dev/prod
    const socketUrl = window.location.hostname === 'localhost' ? 'http://localhost:5000' : 'https://movex-cab.onrender.com';
    const socket = io(socketUrl, {
      auth: { token }
    });

    socket.on('admin:location_update', (data) => {
      setMapData(prev => {
        const updatedDrivers = [...(prev.onlineDrivers || [])];
        const idx = updatedDrivers.findIndex(d => d._id === data.driverId);
        if (idx !== -1) {
          updatedDrivers[idx] = { ...updatedDrivers[idx], location: data.location };
        }
        return { ...prev, onlineDrivers: updatedDrivers };
      });
    });
  ...
  return (
    <div className="-m-8 h-[calc(100vh-80px)] relative bg-gray-100">
      {/* SOS Alert Toast */}
      {sosAlert && (
        <div className="absolute top-4 right-1/2 translate-x-1/2 z-[2000] w-full max-w-lg bg-red-600 text-white p-4 rounded-lg shadow-2xl border-2 border-white flex items-center justify-between animate-bounce">
          <div className="flex items-center gap-3">
            <span className="text-3xl">🚨</span>
            <div>
              <h3 className="font-bold text-lg">EMERGENCY SOS TRIGGERED</h3>
              <p className="text-sm">By User ID: {sosAlert.userId} ({sosAlert.role})</p>
              <p className="text-sm font-semibold">Ride ID: {sosAlert.bookingId}</p>
            </div>
          </div>
          <button onClick={() => setSosAlert(null)} className="text-white hover:text-red-200 font-bold px-3 py-1 bg-red-700 rounded-md">Dismiss</button>
        </div>
      )}
