...
# 3. Replace the Home Tab rendering block
dashboard_ui = """
...
original_map_block_match = re.search(r"(<View style=\{styles\.homeMapPreview\}>.*?\{/\* ── BOTTOM CARD ── \*/\}.*?</View>)", content, re.DOTALL)
if original_map_block_match:
    original_map_block = original_map_block_match.group(1)
    
    bottom_tab_regex = re.compile(r"\{\/\* ─────────────────── BOTTOM TAB BAR \(REDESIGNED\) ─────────────────── \*\/\}.*?\{\/\* ─────────────────── WALLET MODAL ─────────────────── \*\/\}", re.DOTALL)
    ...
    full_replacement = dashboard_ui + "\n              " + original_map_block + "\n            </View>\n          )}\n"
    
    content = content.replace(original_map_block, full_replacement)
